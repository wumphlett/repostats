...
def main():
    ...
    readme_path = next((path for path in github_path.iterdir() if path.stem.startswith("README")), None)
    ...
    with open(Path(os.getenv("GITHUB_WORKSPACE")).resolve() / readme_path.name, "w", encoding="utf-8") as readme_file:
